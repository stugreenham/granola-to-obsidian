...
export function convertProsemirrorToMarkdown(doc: ProseMirrorDoc | null | undefined): string {
  ...
  const processNode = (node: ProseMirrorNode, indentLevel = 0, isTopLevel = false): string => {
    ...
    if (node.content && Array.isArray(node.content)) {
      if (node.type === "bulletList" || node.type === "orderedList") {
        textContent = node.content.map((child) => processNode(child, indentLevel, false)).join("");
    ...
    switch (node.type) {
      ...
      case "orderedList": {
        ...
        const startIndex =
          isOrdered && typeof node.attrs?.start === "number" ? node.attrs.start : 1;
        const items = node.content
          .map((itemNode, i) => {
            if (itemNode.type === "listItem") {
              ...
              const firstText = childContents.find((c) => !c.startsWith("\n")) || "";
              const rest = childContents.filter((c) => c.startsWith("\n")).join("");
  ...
  doc.content.forEach((node) => markdownOutput.push(processNode(node, 0, true)));
